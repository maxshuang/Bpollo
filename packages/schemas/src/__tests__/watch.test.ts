...
import { TriggerConditionSchema, WatchCreationRequestSchema, WatchObjectSchema } from "../watch.js"
  ...
  it("accepts watch with trigger conditions", () => {
    const result = WatchCreationRequestSchema.safeParse({
      ...
      trigger_conditions: [
        ...
        { type: "absence", event_type: "action.resolved", deadline: "2024-06-01T00:00:00.000Z" },
      ],
    })
    expect(result.success).toBe(true)
  })
...
describe("WatchObjectSchema", () => {
  ...
  it("accepts all valid status values", () => {
    const statuses = ["waiting", "triggered", "running", "resolved", "escalated", "expired"] as const
    for (const status of statuses) {
      expect(WatchObjectSchema.safeParse({ ...fullWatch, status }).success).toBe(true)
    }
  })

  it("rejects invalid status", () => {
    expect(WatchObjectSchema.safeParse({ ...fullWatch, status: "active" }).success).toBe(false)
  })

  it("rejects non-UUID watch_id", () => {
    expect(WatchObjectSchema.safeParse({ ...fullWatch, watch_id: "not-a-uuid" }).success).toBe(false)
  })
