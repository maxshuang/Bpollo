  ...
  it("creates a watch and returns 201 with watch_id", async () => {
    vi.mocked(db.insert).mockReturnValue({
      values: vi.fn().mockReturnValue({
        returning: vi
          .fn()
          .mockResolvedValue([{ ...DB_ROW, watchId: "new-watch-id" }]),
      }),
    } as any);
    ...
    vi.mocked(db.update).mockReturnValue({
      set: vi.fn().mockReturnValue({
        where: vi.fn().mockReturnValue({
          returning: vi
            .fn()
            .mockResolvedValue([{ ...DB_ROW, status: "resolved" }]),
        }),
      }),
    } as any);
...
describe("GET /watches", () => {
  ...
  it("returns list of watches for entity", async () => {
    ...
    const res = await app.request(
      "/watches?entity_id=entity-1&tenant_id=tenant-1",
    );
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("returns empty list when no watches found", async () => {
    ...
    const res = await app.request(
      "/watches?entity_id=entity-x&tenant_id=tenant-x",
    );
    expect(res.status).toBe(200);
    const body = await res.json();
