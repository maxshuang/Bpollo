...
const PatchSchema = z.object({
  status:           z.enum(["waiting", "triggered", "running", "resolved", "escalated", "expired"]).optional(),
...
export function buildRouter() {
  ...
  router.post("/watches", async (c) => {
    ...
    logger.info({ watchId: row.watchId, entityId: req.entity_id }, "watch created")
  ...
  router.patch("/watches/:id", async (c) => {
    ...
    if (parsed.data.expected_signals) updates.expectedSignals  = parsed.data.expected_signals
    if (parsed.data.expires_at)       updates.expiresAt        = new Date(parsed.data.expires_at)
